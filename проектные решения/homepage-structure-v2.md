...
КАК МЫ РАБОТАЕМ (с короткой амбровой линией слева, uppercase 13px letter-spacing 0.1em, цвет `#316B4F` — обновлено v2.9 по ДМ20)
...
Use: dark green background (`#316B4F`, палитра Direction A — v2.9); cream/white text; amber CTA button.
...
Фон: тёмно-зелёный `#316B4F` (палитра Direction A, обновлено v2.9 по ДМ20 — осветлён с `#1B4332`).
...
Стилизация: фон полупрозрачный тёмно-зелёный `#316B4F` (обновлено v2.9 по ДМ20) с opacity ~0.95, текст кремовый, ссылка амбровая, кнопка амбровая.

### Плавающая кнопка «Наверх» (ДМ18)

Круглая, справа снизу, диаметр ~50px десктоп / 44px мобильный. Появляется после скролла ~600px, скрывается при возврате в верх страницы. Фон `#316B4F` (обновлено v2.9 по ДМ20), стрелка амбровая `#E4A853`. Плавное появление и исчезновение (fade + translate).

**Замечание к делится с ДМ18:** в поставленном Claude Design макете v16 (23.07.2026) кнопка выполнена в цвете `#1B4332` — на осветлённом фоне футера `#316B4F` этот более тёмный тон работает как визуальный акцент. При полном приведении к обновлённой палитре (`#316B4F` везде) кнопка сливается с фоном футера — требуется UX-компенсация (обводка, тень, или редизайн). Финальное решение — при сборке в Tilda; см. ДМ18 и ДМ20.
